...
<?php include('header.php');?>
		...
		$i=0;
		$statement = $db->prepare("SELECT * FROM order_list WHERE status='1' and member_id='$member_id' ORDER BY order_id DESC");
		$statement->execute();
		$total_pages = $statement->rowCount();
			...
			$statement = $db->prepare("SELECT * FROM order_list WHERE status='1' and member_id='$member_id' ORDER BY order_id DESC LIMIT $start, $limit");
			$statement->execute();
...
<style>
	...
	<?php
					$statement = $db->prepare("SELECT SUM(total),SUM(total_quantity) FROM order_list WHERE status='1' and member_id='$member_id' ");
					$statement->execute();
					$result = $statement->fetchAll(PDO::FETCH_ASSOC);	
					foreach($result as $row)
					{		
