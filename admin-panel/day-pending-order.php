...
if (isset($_POST['day_order'])) {
	...
	if(!(preg_match("/^[A-Za-z0-9]/", $from_date.$to_date)))
	{
		...
		$i=0;
			...
			$statement = $db->prepare("Select * from order_list where status='0' and confirm_code='0' and send_date between '".$from_date."' AND '".$to_date."' ORDER BY order_id DESC ");
			$statement->execute();
...
echo "</style>";
	...
	<?php
		...
		echo "</table>";
					...
					$statement = $db->prepare("SELECT SUM(total),SUM(total_quantity) FROM order_list WHERE status='0' and confirm_code='0' and send_date between '".$from_date."' AND '".$to_date."'");
					$statement->execute();
					$result = $statement->fetchAll(PDO::FETCH_ASSOC);	
					foreach($result as $row)
					{		
