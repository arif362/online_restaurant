...
<?php include('header.php');?>
		...
		<content>
			...
			<p>&nbsp;</p>
				...
				<form method="post" action="monthly-pending-order.php">	
		...
		$i=0;
		$statement = $db->prepare("SELECT * FROM order_list WHERE status='0' and confirm_code='0' ORDER BY order_id DESC");
		$statement->execute();
		$total_pages = $statement->rowCount();
			...
			$statement = $db->prepare("SELECT * FROM order_list WHERE status='0' and confirm_code='0' ORDER BY order_id DESC LIMIT $start, $limit");
			$statement->execute();
	...
	{		
		...
		?>
					...
					<?php
					$statement = $db->prepare("SELECT SUM(total),SUM(total_quantity) FROM order_list WHERE status='0' and confirm_code='0'");
					$statement->execute();
					$result = $statement->fetchAll(PDO::FETCH_ASSOC);	
					foreach($result as $row)
					{		
