	...
	<style>
		...
		<div id="pop">
			<a href="view-successful-order.php"><button id ="close" onclick="document.getElementById('pop').style.display='none'">X</button></a>
				...
				<h2>Search Result of Monthly Successful Order</h2>
<?php
include("config.php");	
...
if (isset($_POST['monthly_order'])) {
	...
	if(!(preg_match("/^[A-Za-z0-9]/", $month.$year)))
	{
		...
		$i=0;
			...
			$statement = $db->prepare("Select * from order_list where status='1' and month='$month' and year='$year' ORDER BY order_id DESC ");
			$statement->execute();
...
echo "</style>";
		...
		echo '<h4>Successful Order of Month: '.$month.' - '.$year.'</h4>';
		echo "<tr>"; 
		echo "<th>Order Id</th>";
...
<script type="text/javascript">
	function PrintContent2()
	{
		...
		echo "</table>";
					...
					$statement = $db->prepare("SELECT SUM(total),SUM(total_quantity) FROM order_list WHERE status='1' and month= '$month' AND '$year'");
					$statement->execute();
					$result = $statement->fetchAll(PDO::FETCH_ASSOC);	
					foreach($result as $row)
					{		
