...
?>
<?php
	$statement = $db->prepare("SELECT count(*) as send_status from message WHERE sender_email='$sub_email' and send_status='1' ");
		$statement->execute();
...
<table class="tbl2" width="100%">
	...
	<?php
		...
		$i=0;
		$statement = $db->prepare("SELECT * FROM message WHERE sender_email='$sub_email' and send_status='1' ORDER BY message_id DESC");
		$statement->execute();
		$total_pages = $statement->rowCount();
			...
			$statement = $db->prepare("SELECT * FROM message WHERE sender_email='$sub_email' and send_status='1' ORDER BY message_id DESC LIMIT $start, $limit");
			$statement->execute();
			$result = $statement->fetchAll(PDO::FETCH_ASSOC);		
